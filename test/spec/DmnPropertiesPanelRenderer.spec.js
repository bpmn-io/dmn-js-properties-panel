...
describe('<DmnPropertiesPanelRenderer>', function() {
  ...
  describe('rendering', function() {
    ...
    it('should rerender on selection changed (single element)', async function() {
      ...
      get(modeler, 'eventBus').on('propertiesPanel.updated', spy);

      // when
      const element = get(modeler, 'elementRegistry').get('dish-decision');

      await act(() => {
        get(modeler, 'selection').select(element);
      });

      // then
      expect(spy).to.have.been.called;
      expect(domQuery('.bio-properties-panel-header-type', propertiesContainer).textContent).to.equal('Decision');
    });
    ...
    it('should rerender on selection changed (multi element)', async function() {
      ...
      // when
      const elements = [
        get(modeler, 'elementRegistry').get('dish-decision'),
        get(modeler, 'elementRegistry').get('elMenu')
      ];

      await act(() => {
        get(modeler, 'selection').select(elements);
      });
    ...
    it('should rerender on selection changed (no element)', async function() {
      ...
      // when
      await act(() => {
        get(modeler, 'selection').select(null);
      });

      // then
      expect(spy).to.have.been.called;
      expect(domQuery('.bio-properties-panel-header-type', propertiesContainer).textContent).to.equal('Definitions');
    });
    ...
    it('should rerender on providers changed', async function() {
      ...
      // when
      await act(() => {
        get(modeler, 'propertiesPanel').registerProvider({
          getGroups: () => {
            return (groups) => {
              return [
                ...groups,
    ...
    it('should rerender on elements changed', async function() {
      ...
      // when
      await act(() => {
        const element = get(modeler, 'elementRegistry').get('definitions_dish');

        get(modeler, 'modeling').updateProperties(element, { name: 'New name' });
      });

      // then
      expect(spy).to.have.been.called;
      expect(domQuery('#bio-properties-panel-name', propertiesContainer).value).to.equal('New name');
    });
  });
});
